...
$app->match('/catalog/{id}', function($id) use($app) {
    ...
    $product_sql['colors'] = $app['db']->fetchAssoc($find_sql, array($product_sql['id']));
    ...
    $find_sql .= "INNER JOIN categories ON category_id = categories.id ";
    $find_sql .= "INNER JOIN providers ON provider_id = providers.id ";
    $find_sql .= "WHERE products.provider_id = $product_sql[provider_id] OR products.category_id = $product_sql[category_id] LIMIT 0,5";
    $rows_sql = $app['db']->fetchAll($find_sql, array());
