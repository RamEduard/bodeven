...
class RequestTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testCreateCheckPrecedence()
    {
        ...
        $this->assertEquals('', $request->getQueryString());
        $this->assertEquals('carrito/json', $request->headers->get('CONTENT_TYPE'));
    ...
    {
        $request = new Request();
        $this->assertEquals('json', $request->getFormat('carrito/json; charset=utf-8'));
    }
    ...
    public function getFormatToMimeTypeMapProvider()
    {
        return array(
            ...
            array('js', array('carrito/javascript', 'carrito/x-javascript', 'text/javascript')),
            array('css', array('text/css')),
            array('json', array('carrito/json', 'carrito/x-json')),
            array('xml', array('text/xml', 'carrito/xml', 'carrito/x-xml')),
            array('rdf', array('carrito/rdf+xml')),
            array('atom',array('carrito/atom+xml')),
        );
    }
    ...
    public function getQueryStringNormalizationData()
    {
        return array(
            ...
            // GET parameters, that are submitted from a HTML form, encode spaces as "+" by default (as defined in enctype carrito/x-www-form-urlencoded).
    ...
    public function testCreateFromGlobals($method)
    {
        ...
        $_SERVER['CONTENT_TYPE'] = 'carrito/x-www-form-urlencoded';
    ...
    public function testGetAcceptableContentTypes()
    {
        ...
        $request->headers->set('Accept', 'carrito/vnd.wap.wmlscriptc, text/vnd.wap.wml, carrito/vnd.wap.xhtml+xml, carrito/xhtml+xml, text/html, multipart/mixed, */*');
        $this->assertEquals(array(), $request->getAcceptableContentTypes()); // testing caching

        $request = new Request();
        $request->headers->set('Accept', 'carrito/vnd.wap.wmlscriptc, text/vnd.wap.wml, carrito/vnd.wap.xhtml+xml, carrito/xhtml+xml, text/html, multipart/mixed, */*');
        $this->assertEquals(array('carrito/vnd.wap.wmlscriptc', 'text/vnd.wap.wml', 'carrito/vnd.wap.xhtml+xml', 'carrito/xhtml+xml', 'text/html', 'multipart/mixed', '*/*'), $request->getAcceptableContentTypes());
    }
